...
- [x] Add a versioned `MemoryDocument` type in `internal/recap` or a focused
      memory package. Include `format_version`, `updated_at_utc`,
      `last_session_id`, `last_message_id`, `last_message_seq`, and the six
      fixed sections.
- [x] Define one entry type with a stable category, text, evidence/reason,
...
- [x] Render the document with application-owned Markdown headings and YAML
...
- [x] Document the exact layout with a checked-in fixture. Keep the file
      readable by a human and easy to search with the existing grep tool.

### 6.2 Validate model input at the boundary

- [x] Define a strict `MemoryEnvelope` containing typed arrays for
...
- [x] `go test ./internal/recap ./internal/db -count=1` exits 0, with the
      command and exit code recorded here.

Evidence: `go test ./internal/recap ./internal/db -count=1` exited 0 on
2026-08-22. The checked-in fixture is
`internal/recap/testdata/memories.md`; the aggregate is rendered and parsed by
`internal/recap/memory.go` and written through the existing contained atomic
writer.
