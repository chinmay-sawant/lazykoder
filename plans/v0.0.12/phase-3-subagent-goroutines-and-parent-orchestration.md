...
> **Status:** implementation and closure evidence validated
...
- [x] Keep all child execution behind `internal/subagent.Manager` and
      `AgentRunner`; the TUI and task tool must not create unmanaged worker
      goroutines.
- [x] Make each `Job` carry the cancellation and completion lifecycle through
...
- [x] Ensure `task_wait` distinguishes a completed summary, partial step-limit
...
- [x] Keep independent task calls in goroutines behind the existing bounded
...
- [x] Define the failure policy for sibling tasks: one failed child must not
...
- [x] Preserve the depth-one rule, role tool allowlists, general-writer lock,
...
- [x] Make `task_cancel` return promptly after signalling one job or all jobs,
      then expose terminal status through `task_status` and `task_wait` after
      cleanup completes.
- [x] Make `/agents` row cancellation use the same manager path as
      `task_cancel`; do not maintain a second UI-only cancellation flag.
- [x] Confirm that shutdown cancels all live jobs, closes provider and tool
      work, waits for owned goroutines to exit, and prevents `Recover` from
      starting duplicate jobs.
- [x] Add race and lifecycle tests for parent cancellation versus child
...
- [x] Multiple child tasks overlap within the configured semaphore limit.
- [x] Parent cancellation, `task_cancel`, and shutdown stop all owned child
      provider requests and leave terminal durable statuses.
- [x] `task_wait` and the `/agents` drawer distinguish completed, partial,
      failed, timed-out, and cancelled results.
- [x] Race and lifecycle tests show no duplicate recovery or leaked goroutine.

## Evidence

- Manager tests cover semaphore overlap, sibling failure, parent cancellation,
  non-blocking cancellation, durable-only cancellation, shutdown, recovery
  races, and manager replacement. Host tests cover terminal result encoding,
  prompt `task_cancel`, and parent-session ownership.
- `GOCACHE=/tmp/lazykoder-phase3-go-cache go test ./internal/subagent ./internal/agent ./internal/ui/chat` passed.
- `GOCACHE=/tmp/lazykoder-phase3-go-cache go test -race ./internal/subagent ./internal/agent ./internal/ui/chat` passed.
- `GOCACHE=/tmp/lazykoder-phase3-go-cache make test` passed.
- `make lint`, `GOCACHE=/tmp/lazykoder-phase3-go-cache make vet`, and
  `GOCACHE=/tmp/lazykoder-phase3-go-cache go build ./...` passed.
