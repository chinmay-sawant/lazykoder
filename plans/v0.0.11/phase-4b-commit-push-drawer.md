...
> **Status:** complete 2026-08-29 (Diff drawer, manual `/diff`, real diff payloads, separated change navigation, auto-expanded file selection, right-side code scrolling, and timer lifecycle coverage)
...
- [x] Clicking it builds the same one-shot agent turn as phase 4.3 (status, diff, recent log, conventional-commit instruction) and executes `git add -A && git commit && git push` via the policy gate. A fresh launch creates the normal chat session before starting that hidden turn. Failures render an alert row. `commitDrawerActionRect` hit-tests the button and delegates to `activateCommitPush`; the prompt `commitActionPrompt` and policy-gated flow remain unchanged.
...
- [x] Gate: `make lint` PASS, `make test` PASS (including drawer, spacing, change-list, mouse, and checker-seam tests), then live TTY check. Verification on 2026-08-29: `make test` ran `go test -p 2 -parallel 2 ./...` and passed; `make lint`, `make vet`, and `make build` passed; focused lifecycle, drawer, spacing, change-list, mouse, and checker-seam tests passed. Real TTY checks at 120x36 and 80x24 confirmed startup rendering. At 120x36, `/diff` opened the file list, Enter opened the selected diff, Escape returned to the separated change list, and the detail view showed the scrollbar and file navigation. The drawer remained visible after 30 and 60 seconds without input and was gone after 95 seconds. No Git commands were run outside the application's existing checker seam or policy-gated action.
