...
- [x] Detect "successful change": a completed assistant turn with zero tool
      ...
      before any git command runs. The production checker is injected from
      `main.go` and tests use the checker seam.
- [x] Add a transient state flag on the chat model (e.g. `pushPromptUntil
      time.Time`) set on detection; nothing persists to SQLite.

## 4.2 Button above the composer

- [x] Render a single-line button row directly above the prompt textarea in
...
- [x] Mouse click activates it (extend `prompt_mouse.go` hit-testing to the
      button row); keyboard shortcut (e.g. `ctrl+g`) as an alternative path.
- [x] Auto-hide: a `tea.Tick` timer hides the button after 90 seconds (within
      the 1-2 minute window) if not clicked; hide immediately after activation.

## 4.3 LLM commit-and-push flow

- [x] On activation, build a one-shot agent turn with a dedicated prompt:
...
- [x] Route the git commands through the same policy gate as other tools so
      ...
      The hidden prompt is wire-only and the normal tool policy remains in
      force.
- [x] No new dependencies; use the existing provider/stream loop.

## 4.4 Docs and gate

- [x] Update `docs/`, keymap cheatsheet, component map, and this knowledge
...
- [x] Gate: `make lint` PASS, `make test` PASS, then live TTY check by user:
      ...
      Automated gates pass; the live repository-action scenario remains a
      human check.
