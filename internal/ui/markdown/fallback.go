...
// renderFallback formats common assistant Markdown using the hand-rolled renderer.
// It is preserved as a zero-dependency fallback for glamour.
func renderFallback(input string, width int) string {
...
func isSeparatorCell(cell string) bool {
	...
	if after, ok := strings.CutPrefix(cell, ":"); ok {
		cell = after
	}
	if before, ok := strings.CutSuffix(cell, ":"); ok {
		cell = before
	}
