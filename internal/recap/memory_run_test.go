...
func TestRunMemoryUpdateRedactsHistoricalCitationsAndSupersedes(t *testing.T) {
	store := openRecapStore(t)
	workdir := t.TempDir()
	session, err := store.CreateSession(context.Background(), db.Session{Directory: workdir, ID: "ses_memory_stale_source"})
	if err != nil {
		t.Fatalf("CreateSession: %v", err)
	}
	for i := 0; i < 4; i++ {
		addRecapMessage(t, store, session.ID, "user", "update the plan", sessionTime(i), true)
	}
	snapshot, err := BuildSnapshot(context.Background(), store, session.ID, SnapshotOptions{Now: sessionTime(4)})
	if err != nil {
		t.Fatalf("BuildSnapshot: %v", err)
	...
	if _, err := WriteMemoryDocument(context.Background(), workdir, previous); err != nil {
	...
	worker := MemoryWorker{
		Client: recapClientFunc(func(_ context.Context, request opencode.ChatRequest) (*opencode.ChatResponse, error) {
			sourceID := snapshot.Messages[0].ID
			if strings.Contains(request.Messages[1].Content, "msg_old") {
				sourceID = "msg_old"
			}
			return &opencode.ChatResponse{
				Content:      `{"preferences":[],"decisions":[],"things_to_avoid":[],"questions":[],"recent_context":[],"supersessions":[{"category":"decisions","existing_text":"Use the old plan","replacement_text":"Use the new plan","evidence":"The current session changed the decision.","source_message_ids":["` + sourceID + `"]}]}`,
				FinishReason: "stop",
			}, nil
		}),
		Model: "deepseek-v4-flash",
	...
	text := string(body)
	if !strings.Contains(text, "Use the old plan") || !strings.Contains(text, "state: \"superseded\"") || !strings.Contains(text, "Use the new plan") {
		t.Fatalf("memory body = %q, want retained superseded fact and active replacement", text)
	}
}

func TestRunMemoryUpdateRepairsUncitedRecentContext(t *testing.T) {
	store := openRecapStore(t)
	workdir := t.TempDir()
	session, err := store.CreateSession(context.Background(), db.Session{Directory: workdir, ID: "ses_memory_repair"})
	if err != nil {
		t.Fatalf("CreateSession: %v", err)
	}
	for i := 0; i < 4; i++ {
		addRecapMessage(t, store, session.ID, "user", "keep the memory update reliable", sessionTime(i), true)
	}
	snapshot, err := BuildSnapshot(context.Background(), store, session.ID, SnapshotOptions{Now: sessionTime(4)})
	if err != nil {
		t.Fatalf("BuildSnapshot: %v", err)
	...
	if got.Status != db.MemoryUpdateStatusCompleted || calls != 2 {
		t.Fatalf("memory update = %+v, provider calls = %d, want completed after one repair retry", got, calls)
	}
	if _, err := os.Stat(filepath.Join(workdir, "knowledge-base", "memories.md")); err != nil {
